...
class LineFriend:
    def __init__(self, talk_file_pass):
        ...
        self.reply_model = self.make_reply_model(
    ...
    def name_talking_with(self):
        names = set([remark[1]
                    for remark in self.remarks if remark[1] != self.name])
        if len(names) > 1:
            print("個人チャットではないので名前を取得できません")
        else:
            return names.pop()
    ...
    def make_reply_model(self, sender, replier):  # senderからrecieverへの返信用辞書
        ...
        for i in range(len(self.remarks) - 1):
            ...
            if sender_remark[1] == sender and replier_remark[1] == replier:
                last_sender_sentence = self.wakati_sentence(
                    sender_remark[2])[-2]
                if last_sender_sentence not in reply_model.keys():
                    reply_model[last_sender_sentence] = []
                reply_model[last_sender_sentence].append(
    ...
    def make_sentence(
        ...
        if reply_to and last_of_reply_to in [key[0] for key in reply_model]:
            key_candidates = [key for key in sentence_model.keys(
            ) if key[0] == "[Begin]" and key[1] in reply_model[last_of_reply_to]]
        else:
            key_candidates = [
                key for key in sentence_model.keys() if key[0] == "[Begin]"]
        if not key_candidates:
            print("Not find Keyword")
            print("reply_model for {}", last_of_reply_to)
            return
